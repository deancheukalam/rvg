			...
			if(!is_array($first_imgs) || !count($first_imgs)) {
				...
				for($a=0; $a<4; $a++) {
					...
					if(isset($first_imgs[$a])) { 
						$img_src = gg_img_src_on_type($first_imgs[$a]['img_src'], $type);	
						$to_display[] = '<img src="'. gg_thumb_src($img_src, $width = 55, $height = 55, 80, $first_imgs[$a]['thumb']) .'" height="55" width="55"/>';		
					}
					else {
						$to_display[] = '';
					}
...
function gg_gall_list_type_filter(){
	...
	?>
	<select name="gg_gall_list_filter">
		<option value=""><?php _e('Any source', 'gg_ml'); ?></option>
		<?php
			$current_v = (isset($_GET['gg_gall_list_filter'])) ? $_GET['gg_gall_list_filter'] : '';
			foreach(gg_types() as $type => $name) {
				printf
					(
...
add_action('restrict_manage_posts', 'gg_gall_list_type_filter');
