...
class Searchfrom_Widget extends WP_Widget {
	...
	function widget( $args, $instance ) {
		...
		?>
			...
			<form method="post" data-withfilter="<?php if( isset( $instance['withfilter'] ) ): echo $instance['withfilter']; else: echo '0'; endif; ?>" data-autoload="<?php echo $instance["autoload"] ?>" action="<?php echo get_permalink( $instance['target'] ); ?>?query=<?php echo $field['name']; ?>">
		...
		<?php			
			if( $element['type'] == 'hierarchical-taxonomies' ):
					...
					<script>
						jQuery( document ).ready( function(){
							loadHierarchicalTaxInputs('<?php echo $key; ?>','<?php echo $_POST[ $key ]; ?>');
						});
					</script>
					<?php
					endif;
			...
			elseif( $element['type'] == 'input' ):
				?>
				<input class="oo2" placeholder="<?php echo $element['fieldname']; ?>" id="sf-field-<?php echo $key; ?>" name="<?php echo $key; ?>" <?php if( isset( $_POST[ $key ] ) && !is_array($_POST[$key]) && $_GET['query'] == 'snel_zoeken_objnr'){ echo ' value="' . $_POST[ $key ] . '" ';}elseif(isset( $_POST[ $key ] ) && !is_array($_POST[$key]) && $_GET['query'] == 'uitgebreid_zoeken'){ echo ' value="' . $_POST[ $key ] . '" '; } ?> />
				<?php	
			...
			?>
				...
				<?php 
					...
					elseif( $data_type == 'meta' && $element['options'] == 'auto' ):
						...
						foreach( $values as $val ):
						?>
						<label><input type="checkbox" <?php if( isset( $_POST[ $key ] ) && is_array($_POST[ $key ]) && in_array( $val->meta_value, $_POST[ $key ] ) ) echo ' checked="checked" '; ?> value="<?php echo $val->meta_value; ?>" name="<?php echo $key; ?>[]" /> <?php echo $val->meta_value; ?></label>					
						<?php
						endforeach;
					elseif( $data_type == 'others' ):
						if( $data_value == 'author' ):
			...
			?>
				<div class="sf-widget-fulltext-wrapper">
					<input class="oo1" <?php if( isset( $_POST[ $key ] ) && !is_array($_POST[ $key ]) && $_GET['query'] == 'snel_zoeken' ) echo ' value="' . $_POST[ $key ] . '" '; ?>placeholder="<?php echo $element['fieldname']; ?>" name="<?php echo $key; ?>" />
				</div>
			<?php
